...
class TelegramDriver:
    ...
    async def send_invite_buttons_to_client(self, client: str):
        invite_markup = TelegramDriver.invite_button(self, telegram_id=client)
        try:
...
class SendInvite(PermissionRequiredMixin, View):
...
class GroupMailing(PermissionRequiredMixin, View):
    permission_required = 'webapp.owner' and 'webapp.manager'
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            if sent_clients:
                message_sent = f"Рассылка отправлена следующим клиентам: {', '.join([client.telegram_id for client in sent_clients])}"
                messages.success(request, message_sent, extra_tags='alert alert-success')
            if unsent_clients:
                message_unsent = f"Следующие клиенты не получили сообщение: {', '.join([client.telegram_id for client in unsent_clients])}"
            ...
            if not clients:
                ...
                messages.warning(request, 'Нет клиентов с telegram_id', extra_tags='alert alert-warning')
        else:
            messages.error(request, 'Ошибка. Форма неверна!', extra_tags='alert alert-danger')
        return redirect('webapp:group_detail', pk=group.pk)


class Mailing(PermissionRequiredMixin, View):
